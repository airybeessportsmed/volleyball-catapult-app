...
export default function AthleteAnalyticsScreen() {
    ...
    return (
      <ScreenContainer className="flex items-center justify-center bg-background p-6">
        <View className="bg-surface rounded-3xl border border-border p-8 items-center justify-center gap-4 max-w-sm shadow-sm">
          ...
          <Text className="text-lg font-bold text-foreground text-center">プロフィール未登録</Text>
      ...
      <ScreenContainer className="bg-background">
        ...
        <View className="flex-1 items-center justify-center p-6 gap-4">
          ...
          <Text className="text-xs text-muted text-center max-w-xs leading-relaxed">
            インポートされたCatapultデータがまだありません。インポート画面からCSVデータを読み込んでください。
          </Text>
        </View>
      </ScreenContainer>
    );
  ...
  const renderACWRGauge = () => {
    ...
    // ACWR Scale: 0.0 to 2.0 (we clamp to this range for UI slider)
  ...
  const renderJumpAnalytics = () => {
    // 5 zones data from latest session
    ...
    return (
      <View className="bg-surface rounded-3xl border border-border p-5 shadow-sm gap-5">
        ...
        {/* Jump Frequency (Jumps per Minute) */}
        {latest.duration && (
  ...
  const renderMenuLoadAnalytics = () => {
    // Menu wise load parsed from rawMenuData
  ...
  const renderSignalLightCard = () => {
    ...
    let statusColorClass = "text-emerald-500 bg-emerald-500/10";
    let statusText = "良好";
    let statusLabel = "🟢";
    if (signal.status === "yellow") {
      bgClass = "bg-amber-50/70 border-amber-200";
      statusColorClass = "text-amber-500 bg-amber-500/10";
      statusText = "注意";
      statusLabel = "🟡";
    } else if (signal.status === "red") {
    ...
    return (
      <View className={`rounded-3xl border p-5 shadow-sm gap-3 ${bgClass}`}>
        <View className="flex-row items-center gap-3">
          <Text className="text-3xl">{statusLabel}</Text>
          <View className="flex-1">
            <Text className="text-sm font-bold text-foreground">
              本日のコンディション判定: {statusText}
            </Text>
        ...
        {!signal.isDataAccumulating && (
          <View className="flex-row justify-around bg-white/75 rounded-2xl py-3 border border-border/30 mt-1">
            <View className="items-center">
              <Text className="text-[9px] text-muted font-bold">ジャンプ</Text>
              <Text className="text-[10px] font-extrabold text-foreground mt-1">
                {signal.jumps === "green" ? "🟢 正常" : signal.jumps === "yellow" ? "🟡 注意" : "🔴 警戒"}
              </Text>
              <Text className="text-[8px] text-muted-foreground mt-0.5">({signal.baselines.jumps.val}回)</Text>
            </View>
            <View className="items-center">
              <Text className="text-[9px] text-muted font-bold">sRPE</Text>
              <Text className="text-[10px] font-extrabold text-foreground mt-1">
                {signal.sRPE === "green" ? "🟢 正常" : signal.sRPE === "yellow" ? "🟡 注意" : "🔴 警戒"}
              </Text>
              <Text className="text-[8px] text-muted-foreground mt-0.5">({signal.baselines.sRPE.val})</Text>
            </View>
            <View className="items-center">
              <Text className="text-[9px] text-muted font-bold">ウェルネス</Text>
              <Text className="text-[10px] font-extrabold text-foreground mt-1">
                {signal.wellness === "green" ? "🟢 正常" : signal.wellness === "yellow" ? "🟡 注意" : "🔴 警戒"}
              </Text>
              <Text className="text-[8px] text-muted-foreground mt-0.5">({signal.baselines.wellness.val}点)</Text>
            </View>
            <View className="items-center">
              <Text className="text-[9px] text-muted font-bold">HRV</Text>
              <Text className="text-[10px] font-extrabold text-foreground mt-1">
                {signal.hrv === "green" ? "🟢 正常" : signal.hrv === "yellow" ? "🟡 注意" : "🔴 警戒"}
              </Text>
              <Text className="text-[8px] text-muted-foreground mt-0.5">({signal.baselines.hrv.val ? `${signal.baselines.hrv.val}ms` : "--"})</Text>
            </View>
          </View>
        )}
      </View>
