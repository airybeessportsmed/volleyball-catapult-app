...
export async function apiCall<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  // - Cookie-based auth is used primarily on Web.
  // - Bearer token is used on Native, and as a fallback on Web to bypass cross-domain port cookie issues.
  ...
  if (sessionToken) {
    ...
    console.log("[API] Authorization header added from stored token");
  }
