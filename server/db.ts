  ...
  try {
    ...
    if (isWellnessOnetap) {
      ...
      if (isWideSoxai) {
        ...
        throw new Error("SOXAI CSV format is invalid or not in wide-format (missing athlete identifiers in headers).");
      }
...
export async function getAthleteAnalytics(athleteId: number, targetDateStr?: string, acwrMetricStr?: string) {
  ...
  const calculateSignalLight = async () => {
    ...
    const metricDefinitions = [
      ...
      { key: "maxJumpHeight", name: "最高ジャンプ高", type: "state" as const },
      { key: "avgJumpHeight", name: "平均ジャンプ高", type: "state" as const },
      { key: "top5JumpHeight", name: "ジャンプ高(Top5平均)", type: "state" as const },
      ...
      { key: "maxAcceleration", name: "最高加速度", type: "state" as const },
...
export async function seedDatabase() {
  ...
  try {
    ...
    // 4. Seeding athlete users & records has been removed to prevent deleted players from re-registering on server restart.
